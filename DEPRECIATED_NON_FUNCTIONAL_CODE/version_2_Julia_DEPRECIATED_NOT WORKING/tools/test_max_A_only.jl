#!/usr/bin/env julia
# Companion to test_max_B_only.jl: J(Φ) = -∫₀ᵀ A(t) dt, λ_F = 0.
#
# This is the §2.6-style trivial test re-run with the integrand swapped
# from B → A. The cost is the A-cost integrand of Eq 37 with the F-barrier
# dropped (λ_F = 0). Unlike the B variant, the A-cost surface is NOT a
# trivially monotone-in-Φ problem — A's drift μ(B,F)·A − η·A³ goes through
# Stuart-Landau and F-coupling nonlinearities before A starts climbing,
# so the optimum is the recovery → overload schedule rather than a
# pinned-high one. §2.7-§2.10 of the writeup pin the residual fp32-bias
# bug in the v2 closed-loop bench specifically to this A-cost surface,
# which is why it's worth running the trivial-A test in the same harness:
# clean kernel, fixed truth params, fixed init state, single open-loop
# plan over the FIXED H_plan = 14 d horizon (§2.11), no filter in the loop.
#
...
#   weights (0, 0, 1) and λ_F = 0 the cost is -∫A dt; at (0, 0, 1) and
...
#    weights (0, 0, 1) and λ_F = 1 the cost reduces to Eq 37; at (0, 0, 1)
#    and λ_F = 0 it is the trivial -∫A dt test (this driver's default). ──
...
if isempty(out_dir)
    ...
    out_dir = joinpath(REPO_ROOT, "outputs", "fsa_high_res",
                        "test_max_A_only", "run_$timestamp")
end
mkpath(out_dir)
...
try
    let plotters_dir = @__DIR__
        ...
        traces_path = joinpath(out_dir,
            "test_max_A_only_T$(T_BENCH_DAYS)d_traces.png")
...
@printf("data.jld2:   %s\n", data_path)
@printf("trace PNG:   %s\n",
        joinpath(out_dir, "test_max_A_only_T$(T_BENCH_DAYS)d_traces.png"))
println("="^72)
