#!/usr/bin/env julia
# Companion to test_max_A_only.jl: J(Φ) = -∫₀ᵀ A(t) dt + λ_F·∫max(F-F_max,0)² dt.
#
# This is Eq 37 byte-for-byte — same as the production controller cost in
# models/fsa_high_res/gpu_control.jl, just exposed through the weighted-
# kernel harness. Default weights (w_B, w_F, w_A) = (0, 0, 1) and λ_F = 1
# turn the F-barrier ON, so the controller now has to balance maximising
# ∫A against the soft cap on F. Compare to test_max_A_only.jl (λ_F = 0):
# the late-bench overload phase should be tempered because pushing F too
# high pays a quadratic penalty.
#
...
#   weights (0, 0, 1) and λ_F = 1 the cost reproduces Eq 37 byte-for-byte
#   (modulo the harmless extra B/F accumulator adds), so this driver also
#   serves as a sanity cross-check against the production kernel.
#
...
#    default weights (0, 0, 1) and λ_F = 1 the cost reduces to Eq 37. ────
...
if isempty(out_dir)
    ...
    out_dir = joinpath(REPO_ROOT, "outputs", "fsa_high_res",
                        "test_max_A_with_F_barrier", "run_$timestamp")
end
mkpath(out_dir)
...
try
    let plotters_dir = @__DIR__
        ...
        traces_path = joinpath(out_dir,
            "test_max_A_with_F_barrier_T$(T_BENCH_DAYS)d_traces.png")
...
@printf("data.jld2:   %s\n", data_path)
@printf("trace PNG:   %s\n",
        joinpath(out_dir, "test_max_A_with_F_barrier_T$(T_BENCH_DAYS)d_traces.png"))
println("="^72)
