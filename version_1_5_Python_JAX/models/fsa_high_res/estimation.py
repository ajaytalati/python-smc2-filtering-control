"""FSA v1.5 estimation — purely-functional. Direct port of
`version_1_5_Julia/models/fsa_high_res/estimation.jl`.
...
Note: `propagate_fn` uses the prior-predictive (no locally-guided
...
from models.fsa_high_res.simulation import (
    DEFAULT_PARAMS, INIT_STATE, PINNED_PARAMS, params_v15_to_v1,
    DT_BIN_DAYS,
)
...
def _params_v15_from_filter_vector(theta_constrained):
    """Build a v1.5-form dict from a length-10 constrained filter vector.
    The 4 pinned values are inserted from the frozen constants.
    """
    return dict(
        tau_B=TAU_B_FROZEN,
...
def obs_log_weight_one(B, F, A, obs, params):
...
def obs_log_weight(particles, obs, params):
    """Vectorised over particles. `particles` shape (M, 3).
    Returns ndarray shape (M,). Direct port of `estimation.jl:91-120`.
    """
    M = particles.shape[0]
    sigma_B = params['sigma_B_obs']
    sigma_F = params['sigma_F_obs']
    sigma_A = params['sigma_A_obs']
    log_norm = (-3.0 * HALF_LOG_2PI
                - np.log(sigma_B) - np.log(sigma_F) - np.log(sigma_A))
    inv2sB2 = 0.5 / (sigma_B * sigma_B)
    inv2sF2 = 0.5 / (sigma_F * sigma_F)
    inv2sA2 = 0.5 / (sigma_A * sigma_A)
    ...
    obs_A = obs['obs_A']
    for m in range(M):
        dB = obs_B - particles[m, 0]
        dF = obs_F - particles[m, 1]
        dA = obs_A - particles[m, 2]
        out[m] = (log_norm
                  - inv2sB2 * dB * dB
                  - inv2sF2 * dF * dF
                  - inv2sA2 * dA * dA)
...
def propagate(particles, Phi_t, params, dt, key):
    """Advance every particle by ONE Euler-Maruyama step under control
    `Phi_t`. Returns a NEW (M, 3) ndarray; does NOT mutate `particles`.

    Pure prior-predictive — no obs information is folded in (v1.5's obs
    is so informative directly on each latent that prior-predictive does
    not degenerate; matches the rationale at `estimation.jl:11-13`).
    ...
    Args:
        ...
        key: JAX PRNGKey; per-particle sub-keys are split deterministically.
    """
    M = particles.shape[0]
    ...
    for m in range(M):
        y = jnp.array(particles[m], dtype=jnp.float64)
        d = drift_jax(y, params_v1, Phi_t)
        sigma = diffusion_state_dep(y, params_v1)
        noise = jax.random.normal(keys[m], (3,), dtype=jnp.float64)
        y_pred = y + d * dt + sigma * sqrt_dt * noise
        out[m, 0] = float(jnp.where(y_pred[0] < 0.0, -y_pred[0],
                          jnp.where(y_pred[0] > 1.0, 2.0 - y_pred[0], y_pred[0])))
        out[m, 1] = float(jnp.abs(y_pred[1]))
        out[m, 2] = float(jnp.abs(y_pred[2]))
    return out
...
def _obs_log_weight_fn_framework(x_new, grid_obs, k, params):
    ...
    `params` is the constrained filter vector. The obs-noise σ's are
    pinned (frozen constants), not in `params`."""
    del params
    obs = dict(
    ...
    return obs_log_weight_one(
        x_new[0], x_new[1], x_new[2],
        obs,
        dict(sigma_B_obs=SIGMA_B_OBS_FROZEN,
