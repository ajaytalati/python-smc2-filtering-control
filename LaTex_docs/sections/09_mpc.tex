...
We close Section~\ref{sec:mpc} by setting out a second controller --
LQG with a backward-differential Riccati gain -- that has now been
implemented as Stage~H of the project (\texttt{smc2fc/control/lqg/}) and
benchmarked against the SMC${}^2$ MPC. The sketch below makes the
assumptions and limitations explicit; the empirical comparison is in
Section~\ref{sec:results-lqg} and the verdict (the cubic Stuart--Landau
non-linearity is essential, no LQG variant matches the full SMC${}^2$
machinery on this model) in Section~\ref{sec:discussion-lqg}.
...
\paragraph{Implementation in the project.}
The Stage~H module \texttt{smc2fc/control/lqg/} provides
\begin{enumerate}
  \item \texttt{linearize.py} -- automatic linearisation of the drift in
        \texttt{version\_2/models/fsa\_high\_res/\_dynamics.py} via JAX
        \texttt{jacfwd}, evaluated at the operating point
        $(B_{\mathrm{typ}},F_{\mathrm{typ}},A_{\mathrm{typ}},\Phi=1)$;
  \item \texttt{riccati.py} -- a fourth-order Runge--Kutta backward
        integrator for \eqref{eq:riccati} on the project's outer grid
        (15-min or 1-h depending on \texttt{FSA\_STEP\_MINUTES});
  \item \texttt{controller.py} -- \texttt{LQGController} emitting both
        the open-loop optimal $\Phi$ schedule and a closed-loop feedback
        gain $K(t)$ for use against an SMC${}^2$ posterior-mean state.
\end{enumerate}
The bench driver \texttt{version\_2/tools/bench\_lqg\_baseline\_fsa.py}
mirrors the SMC${}^2$ checkpoint schema and writes a comparable manifest,
so the two controllers can be joined by horizon and grid for the
comparison plotted in Section~\ref{sec:results-lqg}.

\paragraph{Three structural facts of the linearisation.}
At the operating point the JAX-autodiff linearisation produces
\begin{equation}
  A_{\mathrm{lin}} =
  \begin{pmatrix}
...
\begin{enumerate}
  \item[(S1)] $B_{\mathrm{lin}}^{(A)} = 0$. The training stimulus $\Phi$
    has \emph{no direct linear path} to the autonomic amplitude $A$.
    Linearised LQG can only steer $A$ via the slow detour
    $\Phi \to F \to A$.
  \item[(S2)] One eigenvalue of $A_{\mathrm{lin}}$ is positive
    ($+0.003$). At $A_{\mathrm{typ}} = 0.10$ the cubic damping $-\eta A^3$
    contributes only $3\eta A_{\mathrm{typ}}^2 = 0.006$ to the
    $A$-direction rate, less than the destabilising contribution from
    the $\mu$-coupling. The linearisation is therefore \emph{unstable}
    in a slow $A$-direction; the cubic non-linearity that restores
    stability in the true model is not present in the linearised state
    space.
  \item[(S3)] $A_{\mathrm{lin}}^{(A,F)} = -0.026 < 0$. In the
    linearised state space, raising $F$ \emph{reduces} $A$, the
    \emph{opposite} of the true plant's coupling
    (raising $F$ raises $A$ via the slow build of $\mu$ before the
    quadratic suppression takes over). The first-order Taylor
    expansion at $A_{\mathrm{typ}}$ inverts the sign of the
    physiologically relevant cross-coupling.
\end{enumerate}
No choice of $(Q,R,Q_T,x_{\mathrm{ref}})$ within the LQG family can
correct these structural facts -- they are what
Assumption~\ref{ass:lqg-A1} produces. They are the empirical reason for
the failure of LQG at long horizons reported in
Section~\ref{sec:results-lqg}.
