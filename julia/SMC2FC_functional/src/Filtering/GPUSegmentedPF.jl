...
GPU-resident segmented particle-filter primitives. Shared across all GPU
PF models. Mirrors the framework-side path of
`smc2fc/filtering/gk_dpf_v3_lite.py`.

Capabilities:

- Per-chain weight reductions (max, log Z, ESS, weighted mean, normalised
    weights, cumulative sum).
- Per-chain systematic resampling with deterministic offset (CRN-friendly).
- Liu–West shrinkage between segments.
- OT sigmoid-blend rescue (Nyström + low-rank Sinkhorn) using the
    framework's `OT.jl` primitives on `CuArray` inputs.

All kernels are based on KernelAbstractions / CUDA.jl; particles, log
weights, and normalised weights stay GPU-resident across the full window.
CPU is only touched for kernel launches and to read the per-chain
log-likelihood at the end of a window.

Models supply ONLY their per-particle propagate kernel.

# Functional API note

The kernels in this module are inherently imperative — they write into
output `CuArray`s passed as arguments. This is a property of GPU code,
not a Pythonic-port artefact. Callers wrap these kernels in a
functional-API surface at the higher layer (e.g.
`run_segmented_smc_step!`) by accepting a `GPUSegmentedBuffers`
container as a single argument and not exposing the per-array `!`
mutation to higher layers.
"""
