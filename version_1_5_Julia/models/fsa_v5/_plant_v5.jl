...
function plant_step_v5(state::PlantState6D,
    ...
    sde_noise = SVector{6, Float64}(
        randn(sde_rng), randn(sde_rng), randn(sde_rng),
        randn(sde_rng), randn(sde_rng), randn(sde_rng),
    )

    sigma_diag = _sigma_diag_from_params(params)
    new_state  = em_step_v5(state.state, phi, params, sigma_diag, dt, sde_noise)
    ...
    t_day = state.t_bin / BINS_PER_DAY
    C     = cos(2.0 * pi * t_day)
...
5-channel obs + bimodal Φ + circadian regressor):
    ...
    trajectory  :: Matrix{Float64} (n, 6)         — post-step states
...
function plant_rollout_v5(state0::PlantState6D,
    ...
    @inbounds for k in 1:n
        ...
        obs_HR[k]      = Float32(out.obs.obs_HR)
        obs_sleep_v[k] = Float32(out.obs.obs_sleep ? 1.0 : 0.0)
        obs_S_v[k]     = Float32(out.obs.obs_S)
        obs_steps_v[k] = Float32(out.obs.obs_steps)
        obs_VL_v[k]    = Float32(out.obs.obs_VL)
        ...
        t_day = s.t_bin / Float64(BINS_PER_DAY)
        C_v[k] = Float32(cos(2.0 * pi * t_day))
    end
...
Build a plant initial state from the canonical `DEFAULT_INIT`
("deconditioned but otherwise healthy" per tech guide §9.10).
"""
function init_plant_state_v5(; t_bin::Int = 0)
    s = SVector{6, Float64}(
        DEFAULT_INIT.B,   DEFAULT_INIT.S,   DEFAULT_INIT.F,
        DEFAULT_INIT.A,   DEFAULT_INIT.KFB, DEFAULT_INIT.KFS,
    )
...
function _sample_obs(state::SVector{6, Float64}, C::Float64,
