#!/usr/bin/env bash
# Julia v1.5 horizon sweep launcher — "filter HMC unified with framework
# ChEES path" study (2026-05-09).
#
# Variant of run_julia_horizon_sweep_no_filter_hmc.sh with:
#   * Filter HMC ON, using the framework's parallel_hmc_one_move_generic!
#       + chees_pick_L_generic (same code path as the controller HMC).
#       β-tempering correctly applied via beta=next_λ.
#   * --hmc-step-size 0.005   (sweep sweet spot; was 0.05 default)
#   * --h-fd 0.01             (sweep sweet spot; was 1e-3 hardcoded)
#   * --hmc-leapfrog 4        (initial fallback; ChEES picks the
#                              actual L per level from the candidate list)
#   * --filter-chees-max 64   (candidate list = [4, 8, 16, 32, 64])
#   * All filter rejuvenation tricks ON (LW=0.97, Silverman+KDE 1.0,
#       Gaussian bridge true).
#   * Fast (reduced) controller config
...
SWEEP_ROOT="$REPO/compare_v15_julia_vs_python/example_run/julia_horizon_sweep_2026-05-09_filter_hmc_unified"
