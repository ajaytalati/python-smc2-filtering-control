#!/usr/bin/env bash
# Julia v1.5 horizon sweep launcher — "controller exploration aids
# stacked, T=14d only" study (2026-05-09).
#
# Same flag set as run_julia_horizon_sweep_filter_hmc_unified.sh except
# four controller-side exploration aids are stacked:
#   * --ctrl-num-mcmc 24       (was 8: 3× more independent HMC moves
#                                per level — the picker's L=16 verdict
#                                says short fresh-momentum moves dominate)
#   * --ctrl-sigma-prior 2.5   (was 1.5: broader initial cloud)
#   * --ctrl-max-lambda-inc 0.05  (was 0.20: 4× finer β-ladder, more
#                                   tempering levels per replan)
#   * --ctrl-target-ess-frac 0.3  (was 0.5: reinforces the finer ladder)
#   * --ctrl-n-inner 128       (was 64: 2× CRN trials per cost call,
#                                cuts cost-gradient noise by ~√2)
#
# Default T_LIST is just (14) — single horizon sanity test. T=14d is
# the hardest horizon for the FSA-v1 cost (shortest planning window).
...
SWEEP_ROOT="$REPO/compare_v15_julia_vs_python/example_run/julia_horizon_sweep_2026-05-09_ctrl_explore_T14"
