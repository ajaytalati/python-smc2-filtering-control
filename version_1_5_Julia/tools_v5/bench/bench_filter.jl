...
#       `gpu_log_density_v5` call inside `run_outer_smc`.
#
...
# `Printf.@sprintf`, the FSA GPU kernels (`gpu_log_density_v5`,
# `FSAv5GPUTarget`), and the framework's `parallel_hmc_one_move_generic!`,
...
function run_outer_smc(target::FSAv5GPUTarget,
                       grid_obs::NamedTuple,
                       n_smc::Int,
                       cfg::NamedTuple,
    ...
    n_temp = 0
    while λ < 1.0 - 1e-6
        ll_data = gpu_log_density_v5(target, U, grid_obs, hash((key, :ll, n_temp)))
        ...
        #
        # `log_density_fn` wraps `gpu_log_density_v5` into the
        ...
        n_acc_total = 0
        log_density_fn = U_in -> gpu_log_density_v5(target, U_in, grid_obs,
    ...
    # Final gpu_log_density_v5 call on the posterior chains so that
    ...
    _ = gpu_log_density_v5(target, U, grid_obs, hash((key, :final)))
...
# stats kernel populates `bufs.mu_per_chain[1:M, :]` (M, 6) with weighted
# means per chain (v5 state is 6D: [B, S, F, A, K_FB, K_FS]). We
# marginalise across M with uniform weights since each filter chain
# represents one θ posterior particle.

function extract_xhat(target::FSAv5GPUTarget, M::Int)
    mu_cpu = Array(view(target.bufs.mu_per_chain, 1:M, :))
    return SVector{6, Float64}(mean(mu_cpu[:, 1]),
                                mean(mu_cpu[:, 2]),
                                mean(mu_cpu[:, 3]),
                                mean(mu_cpu[:, 4]),
                                mean(mu_cpu[:, 5]),
