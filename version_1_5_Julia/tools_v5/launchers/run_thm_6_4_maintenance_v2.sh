#!/usr/bin/env bash
# FSA v5 controllability test — Theorem 6.4 (Constructive maintenance).
#
# Tests the SMC²-MPC controller (full closed-loop stack) starting from the
# v2 trained-athlete state with the baseline policy at the maintenance
# Φ = (1, 1).
#
# Question: does the controller hold maintenance Φ ≈ (1, 1) and keep A near
# A* = 1.238, or does it drift?
#
# Cost: simplified J = -A_acc + lam_island * island_acc.
# Truth params: TRUTH_PARAMS_V5_RECOMMENDED_V2.
# Reference deterministic outcome (PDF Thm 6.4): A(100) = 1.246 under
# Φ ≡ (1, 1). The bench's BASELINE rollout reproduces this; the CLOSED-LOOP
# trajectory is what the controller actually does.
#
# Default T=100d.
...
RUN_DIR="$OUTPUT_BASE/${TIMESTAMP}_thm_6_4_maintenance_v2_T${T_DAYS}d"

mkdir -p "$RUN_DIR"
cd "$JULIA_DIR" || exit 1

echo "============================================================"
echo "THM 6.4 — constructive maintenance (closed-loop, v2)  T=${T_DAYS}d"
