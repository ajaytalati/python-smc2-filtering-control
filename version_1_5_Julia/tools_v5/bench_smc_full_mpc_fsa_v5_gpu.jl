...
using .FSAv5
using .FSAv5.PlantV5: PlantState6D, plant_rollout_v5, init_plant_state_v5,
                       init_plant_state_trained
using .FSAv5.SimulationV5: BINS_PER_DAY, DT_BIN_DAYS,
                            TRUTH_PARAMS_V5, DEFAULT_OBS_PARAMS_V5,
...
function main(args::Dict{String,Any})
    ...
    # ── Filter prior (unconstrained, v5 PARAM_PRIOR_CONFIG_V5) ──
    prior_means  = Float64[m for (_, _, m, _) in PARAM_PRIOR_CONFIG_V5]
    prior_sigmas = Float64[s for (_, _, _, s) in PARAM_PRIOR_CONFIG_V5]
    ...
    phi_max = 3.0   # mirrors gpu_control_v5.jl Phi_max default
    (0.0 <= init_phi_B <= phi_max) || error(
        "--init-phi-B must be in [0, $phi_max], got $init_phi_B")
    (0.0 <= init_phi_S <= phi_max) || error(
    ...
    initial_plan_B, initial_plan_S = if is_open_loop
        @info "OPEN-LOOP: building one initial plan from $init_preset+TRUTH_PARAMS_V5..."
        s0 = SVector{6, Float64}(
        ...
        (fill(Float32(init_phi_B), T_total_bins),
         fill(Float32(init_phi_S), T_total_bins))
    end
