...
def make_gk_dpf_v3_lite_log_density_compileonce(
    ...
    # Per-state bound vectors used by every clip site (replaces the
    # imperative `for i, (lo, hi) in enumerate(model.state_bounds): …`
    # patterns with one vectorised `jnp.clip(arr, lo_arr, hi_arr)`).
    lo_arr = jnp.array([b[0] for b in model.state_bounds],
                       dtype=jnp.float64)
    hi_arr = jnp.array([b[1] for b in model.state_bounds],
                       dtype=jnp.float64)
    ...
    def log_density(u, grid_obs, fixed_init_state, w_start, key0):
        ...
        particles = jnp.clip(particles, lo_arr, hi_arr)
        ...
        def scan_step(carry, k):
            ...
            if ot_max_weight >= 1e-6:
                ...
                ot_safe = jnp.clip(ot_safe, lo_arr, hi_arr)
            ...
            particles_next = jnp.clip(particles_next, lo_arr, hi_arr)
