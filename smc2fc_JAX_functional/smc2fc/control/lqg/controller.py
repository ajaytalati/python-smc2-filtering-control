...
class LQGController:
    ...
    def nominal_trajectory(self) -> np.ndarray:
        ...
        x0 = np.asarray(self.spec.x_star, dtype=np.float64)
        ...
        # Replaces the imperative `for _ in range(...): traj.append(...)`
        # of the source with a `functools.reduce` fold that returns the
        # full trajectory as a numpy array, no list mutation.
        def fold(traj_so_far: np.ndarray, _: int) -> np.ndarray:
            x_next = np.asarray(step(traj_so_far[-1], self.spec.phi_ref))
            return np.vstack([traj_so_far, x_next[None, :]])

        return functools.reduce(
            fold, range(int(self.spec.n_steps)), x0[None, :]
        )
