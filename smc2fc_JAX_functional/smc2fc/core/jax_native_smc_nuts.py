...
to eliminate per-stride JIT recompilation. (NUTS version)
...
The NUTS kernel itself is built ONCE at module load via
`blackjax.mcmc.nuts.build_kernel()`, which is fine because that's a
...
# Module-level NUTS kernel — compiled once when this module imports.
NUTS_KERNEL = nuts.build_kernel()
NUTS_INIT = nuts.init
...
# =========================================================================
# One tempered SMC step: reweight + resample + NUTS moves.
# =========================================================================
...
def _nuts_step_chain(initial_position, logpost_fn, num_steps,
                     step_size, inv_mass, max_num_doublings, rng_key):
    """Apply num_steps NUTS moves to a single particle's position.
    Pure JAX, jit-friendly.
    """
    # Initialise NUTS state from position.
    init_state = NUTS_INIT(initial_position, logpost_fn)

    def body(carry, _):
        state, key = carry
        key, sub = jax.random.split(key)
        new_state, _info = NUTS_KERNEL(
            sub, state, logpost_fn,
            step_size, inv_mass, max_num_doublings=max_num_doublings)
...
def run_tempered_chain(
    ...
    return _run_tempered_chain_impl(
        ...
        num_mcmc_steps, hmc_step_size, max_num_doublings,
    )
...
def _run_tempered_chain_impl(
    ...
    def body_fn(carry):
        ...
        key, k_resample, k_nuts = jax.random.split(key, 3)
...
_run_tempered_chain_jit = jax.jit(
    ...
    static_argnames=("num_mcmc_steps", "max_num_doublings"),
)
...
def run_smc_window_native(
    ...
    t0 = time.time()
    max_num_doublings = getattr(cfg, 'nuts_max_num_doublings', 3)
