...
\subsection{A note on the linear alternative}\label{sec:mpc-lqg-pointer}

For completeness we examined a closed-form linear alternative --
linear-quadratic-Gaussian (LQG) control with a backward differential
Riccati gain -- as a baseline. Such a controller is built on a
linearisation of the FSA-v2 drift around a fixed operating point,
which by construction \emph{discards the cubic Stuart--Landau term}
$-\eta A^3$ that sits at the heart of the model's autonomic
dynamics. The resulting linearised state space exhibits structural
defects (no direct linear path from $\Phi$ to $A$, an unstable
$A$-eigenvalue absent the cubic, and an inverted sign on the
$F\!\to\!A$ coupling) that no choice of cost weights can repair. The
LQG schedule and its empirical performance against the SMC${}^2$ MPC
are recorded in Appendix~\ref{app:lqg} for completeness; on this plant
the LQG controller does not match the SMC${}^2$ MPC, and we do not
pursue it further in the main text.
